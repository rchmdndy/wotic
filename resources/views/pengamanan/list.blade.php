    ...
    <section class="container mt-3">
        ...
        @elseif(empty($destinationList))
            <h5 class="display-5 text-center justify-content-center position-absolute top-50 start-50 translate-middle">
                Belum ada data terkait {{$jenis_wisata}}
            </h5>
        @else
        <h1 class="display-6 my-5 text-center"><span class="text-capitalize">{{$jenis_wisata}}</span> yang tersedia di Wonosobo</h1>
...
{{--                                <p class="card-subtitle"><i class="bi bi-geo-fill"></i> Dari Jogja: ~{{ $destinasi['jarak_dari_jogja'] }} KM</p>--}}
                            ...
                            <a href="{{ route('pengamanan.detail', ['id' => $destinasi['id']]) }}" class="btn btn-primary btn-block">
